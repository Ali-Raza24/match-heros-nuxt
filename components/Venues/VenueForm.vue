    ...
    <el-form :class="'xl:max-w-[700px] max-w-full xl:pr-20 block mx-auto'" :model="store.form" label-position="top"
        ...
        <el-row :gutter="30">
            ...
            <el-col :sm="16" :xs="24">
                <div class="relative">
                    <el-upload
                        ...
                        <el-avatar :class="'relative z-10'" shape="square" :size="200" fit="fill"
                            :src="displayedBanner" />
            ...
            <el-col :span="24" :sm="24" :xs="24">
                <el-form-item label="Add Vanue Location" prop="address"
                    @click="!isSubmit ? handleOpenMapModal() : null">
                    <el-input :class="[isSubmit ? 'hasValue' : null]" class="locationField" v-model="store.form.address"
                        placeholder="Add Venue Location" size="large" :prefix-icon="Location" readonly />
                    <el-button :class="'btn-link absolute top-2 right-5 !bg-transparent !border-0 !px-0'" type="success"
                        @click="handleOpenMapModal">Change</el-button>
                </el-form-item>
            </el-col>
            <el-col :sm="12" :xs="24">
            ...
            <el-col :sm="12" :xs="24">
                <el-form-item label="€ Off Peak" prop="off_peak">
                    <el-input @input="onlyNumbers($event, 'off_peak')" v-model="store.form.off_peak" />
                </el-form-item></el-col>
            <el-col :sm="12" :xs="24">
                <el-form-item label="€ On Peak" prop="on_peak">
                    <el-input @input="onlyNumbers($event, 'on_peak')" v-model="store.form.on_peak" />
                </el-form-item></el-col>
            <el-col :sm="12" :xs="24">
                <el-row :gutter="20">
                    <el-col :span="12">
        ...
        <el-form-item>
            <el-button :class="'btn-theme'" type="success" :disabled="store.loading" v-loading="store.loading"
                @click=" store.buttonText === 'UPDATE A VENUE' ? updateForm(ruleFormRef) : onSubmit(ruleFormRef)"> {{
                    store.buttonText
                }} </el-button>
        </el-form-item>
    </el-form>
    <ModalsGoogleMapModal :address="address" :visible="visible" :lat="store.form.latitude" :lng="store.form.longitude"
        @close="closeDialog" @input-address="getAddress" />
</template>
...
const beforeAvatarUpload = (rawFile) => {
    if (rawFile.size / 1024 / 1024 > 2) {
        ElMessage.error('Picture size cannot exceed 2MB!');
        return false;
    }
    return true;
};
...
const getAddress = (addressData) => {
    ...
    store.form.latitude = addressData.latLng.lat;
    store.form.longitude = addressData.latLng.lng;
    store.form.address = addressData.address;
    store.form.venue_location = addressData.address;
